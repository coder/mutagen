...
func invoke(topLevelFlags []string, cmd string, arguments []string) error {
	...
	if cmd != "" {
		composeArguments = append(composeArguments, cmd)
...
func invokeAndExit(topLevelFlags []string, cmd string, arguments []string) {
	// Run invoke. If there's no error, then we can exit successfully as well.
	err := invoke(topLevelFlags, cmd, arguments)
	if err == nil {
		os.Exit(0)
	}
...
func handleTopLevelFlags() {
	...
	if composeConfiguration.skipHostnameCheck {
		command.Fatal(errors.New("--skip-hostname-check flag not supported by Mutagen"))
	}
}
