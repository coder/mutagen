...
func PauseWithSelection(
	...
	prompter, promptingErrors, err := promptingsvc.Host(
		...
		&command.StatusLinePrompter{Printer: statusLinePrinter}, false,
	)
	if err != nil {
		promptingCancel()
	...
	request := &forwardingsvc.PauseRequest{
		Prompter:  prompter,
		Selection: selection,
	}
	response, err := forwardingService.Pause(context.Background(), request)
	promptingCancel()
	<-promptingErrors
	if err != nil {
...
var pauseCommand = &cobra.Command{
	Use:          "pause [<session>...]",
	Short:        "Pause a forwarding session",
	RunE:         pauseMain,
	SilenceUsage: true,
}
