...
func CreateWithSpecification(
	...
	statusLinePrinter := &command.StatusLinePrinter{}
	...
	prompter, promptingErrors, err := promptingsvc.Host(
		...
		&command.StatusLinePrompter{Printer: statusLinePrinter}, true,
	)
	if err != nil {
		promptingCancel()
...
func createMain(_ *cobra.Command, arguments []string) error {
	...
	if globalConfigurationNonExistent {
		...
		if err != nil {
			...
			configuration = synchronization.MergeConfigurations(configuration, globalConfiguration)
			command.Warning("TOML-based global configuration files are deprecated, please migrate to YAML")
		}
	}
	...
	if createConfiguration.configurationFile != "" {
		if filepath.Ext(createConfiguration.configurationFile) == ".toml" {
			command.Warning("TOML-based configuration files are deprecated, please migrate to YAML")
